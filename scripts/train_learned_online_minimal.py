...
def train(
    *,
    ...
    for episode in range(int(episodes)):
        task = _make_task(int(rng.integers(3)), seed=seed + episode)
        ...
        for _step in range(int(max_steps)):
            ...
            action = str(actions[int(rng.integers(len(actions)))])
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("artifacts/learned_online_minimal_latest.pkl"))
