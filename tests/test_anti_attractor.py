...
def test_local_button_cycle_is_vetoed_after_diverse_prefix() -> None:
    gate = HardAntiAttractorGate(window=12, threshold=0.80)
    obs = _obs()
    for action in ["1", "2", "3", "4", "1", "3", "2", "4", "5", "4"]:
        gate.observe_transition(obs, action, _result(obs))

    selected, diagnostics = gate.select_action("5", ["5", "2", "click:1:1"], obs)

    assert selected == "click:1:1"
    assert diagnostics["vetoed_actions"][0]["action"] == "5"
    assert any("local_period_2" in reason for reason in diagnostics["vetoed_actions"][0]["reasons"])
...
def test_hard_veto_fallback_does_not_resurrect_suppressed_contact_action() -> None:
    class _Base:
        def choose_action(self, observation: _Obs) -> tuple[str, dict[str, Any]]:
            del observation
            return "5", {"action_scores": {"5": 9.0, "click:1:1": 8.0, "1": 1.0}, "policy": {}}
    ...
    class _Memory:
        action_counts: dict[str, int] = {"5": 0, "1": 0, "click:1:1": 0}
        active_sequence_source = ""
        positive_prefix_completed = False
        best_event_prefix: list[str] = []
    ...
    controller.post_prefix_rng = jepa_arc_eval.random.Random(0)
    controller.anti_attractor = HardAntiAttractorGate(window=8, threshold=0.80)
    obs = _obs(actions=("5", "click:1:1", "1"))
    for _ in range(4):
        controller.anti_attractor.observe_transition(obs, "5", _result(obs))
    ...
    policy = diagnostics["jepa_policy"]
    assert policy["pre_veto_chosen_action"] == "5"
    assert policy["public_no_effect_suppressed_count"] == 1
    vetoed_actions = [item["action"] for item in policy["hard_anti_attractor"]["vetoed_actions"]]
    assert "click:1:1" not in vetoed_actions
