...
Terminal outcome: **AUDIT NOT PROVEN**
...
| `src/curriculum.py` | `1264678AEA2716779D18EB0217CB18947FD4EFC6658E96AAAB9B32B9E6A5B72E` | `1264678AEA2716779D18EB0217CB18947FD4EFC6658E96AAAB9B32B9E6A5B72E` | True |
...
| Curriculum growth changes weights and retains old task behavior | WEAK | Compared pre/post curriculum weights, generated-private curriculum accuracy, and old-task retention. |
...
```json
{
  "changed_parameter_tensors": 20,
  "parameter_l2_total": 15.083510655909777,
  ...
  "old_task_retention_after": {
    "action_accuracy": 0.36611328125,
    "delayed_memory_accuracy": 0.91796875,
    "object_color_accuracy": 0.9812911184210527,
    "object_pos_accuracy": 1.0,
    "object_permanence_accuracy": 0.9906455592105263,
    "provenance_accuracy": 0.29150390625,
    "grounded_language_accuracy": 0.54296875,
    "self_world_continuity_accuracy": 0.25390625,
    "core_mean": 0.6680497018914474
  },
  "old_task_core_delta": -0.32740928099343647,
  "old_task_retention_passes": false
}
```
...
### curriculum_update
`src/curriculum.py:81`

```python
81:         private_loss = F.cross_entropy(
82:             outputs["private_logits"].reshape(-1, outputs["private_logits"].shape[-1]),
83:             batch["private_target"].reshape(-1),
84:         )
85:         retention = F.cross_entropy(
86:             outputs["world_pos_logits"].reshape(-1, outputs["world_pos_logits"].shape[-1]),
87:             batch["world_pos_target"].reshape(-1),
88:         )
89:         loss = language_loss + 0.25 * private_loss + 0.05 * retention
90:         optimizer.zero_grad(set_to_none=True)
91:         loss.backward()
92:         torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
93:         optimizer.step()
94:         final_loss = float(loss.detach().item())
95:     model.eval()
96:     after = curriculum_accuracy(model, concept_id, seed=8300 + concept_id, device=device)
97:     save_checkpoint(output, model, {"curriculum_concept_id": concept_id, "steps": steps}, {"curriculum_after": after})
98:     return {
99:         "concept_id": float(concept_id),
100:         "steps": float(steps),
101:         "accuracy_before": before,
```
...
## Limitations
- Curriculum retention failed: generated-private new concept accuracy 1.000000, old-task core delta -0.327409, old-task core after 0.668050.
